...
export function FullScreenWrapper({ children, className }: FullScreenWrapperProps) {
  ...
  return (
    <>
      {/* Toggle button (rendered in normal flow for inline usage) */}
      <Button
        variant="outline"
        size="sm"
        onClick={toggle}
      ...
      <AnimatePresence>
        {isFullScreen && (
          ...
          >
            ...
            {/* Content area */}
            <div className={cn("flex-1 overflow-auto p-4 scrollbar-thin", className)}>
              {children}
            </div>
          </motion.div>
        )}
      ...
      {/* Normal flow content (always rendered, visible when not full-screen) */}
      {!isFullScreen && (
