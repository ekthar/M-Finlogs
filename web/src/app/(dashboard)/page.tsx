...
interface DashboardData {
  todaySales: number; monthlySales: number; cashBalance: number;
  bankBalance: number; receivables: number; totalFunds: number;
}
...
function fmt(n: number): string { return new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", minimumFractionDigits: 2 }).format(n); }
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/dashboard").then((r) => r.json()).then((d) => setData(d)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <motion.div variants={stagger} initial="initial" animate="animate" className="space-y-6">
      <motion.div {...fadeUp}>
        ...
        <p className="mt-1 text-sm text-zinc-500">Overview of your financial activity</p>
      </motion.div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
              ...
              <div><p className="text-xs text-zinc-500">Today Sales</p><p className="mt-1 text-lg font-semibold text-zinc-900 dark:text-zinc-100">{fmt(d.todaySales)}</p></div>
