...
export default function DayBookPage() {
  ...
  const handlePdfExport = () => {
    ...
    try {
      ...
      exportToPdf({
        title: "Day Book",
        subtitle: `Date: ${new Date(date).toLocaleDateString("en-IN")} | ${entries.length} entries | Total: ₹${fmt(total)}`,
        headers: ["Date", "Bill No", "Party", "Type", "Mode", "Amount (₹)"],
        rows: entries.map(e => [new Date(e.date).toLocaleDateString("en-IN"), e.billNo || "—", e.party, e.txnType, e.paymentMode, `₹${fmt(e.amount)}`]),
  ...
  const handleExcelExport = () => {
    ...
    const data = entries.map(e => ({ Date: new Date(e.date).toLocaleDateString("en-IN"), Bill: e.billNo || "—", Party: e.party, Type: e.txnType, Mode: e.paymentMode, Amount: e.amount }));
    exportToExcel(data, `DayBook_${date}`);
    ...
    <motion.div initial="initial" animate="animate" className="space-y-5">
      <motion.div {...fadeUp} className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handlePdfExport} disabled={pdfLoading || !entries.length}><FileText className="mr-1.5 h-3.5 w-3.5"/>{pdfLoading ? "..." : "PDF"}</Button>
          <Button variant="outline" size="sm" onClick={handleExcelExport} disabled={!entries.length}><Download className="mr-1.5 h-3.5 w-3.5"/>Excel</Button>
